...
function munsellHvcToRgb(hue, value, chroma) {
  const m = getMunsell()
  // If hue looks like a full Munsell string ("5Y 7.3/2.7"), use it directly
  const str = typeof hue === 'string' && hue.includes('/') ? hue : `${hue} ${Number(value).toFixed(1)}/${Number(chroma).toFixed(1)}`
