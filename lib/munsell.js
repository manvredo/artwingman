...
function munsellHvcToRgb(hue, value, chroma) {
  try {
    ...
    if (typeof hue === 'string') {
      // String hue like "4PB" — convert via munsellToMhvc
      const converted = munsell.munsellToMhvc(hue + ' ' + value + '/' + chroma)
      ;[hue100, v, c] = converted
    } else {
      hue100 = hue
      v = value
      c = chroma
    }
    const [rr, gg, bb] = munsell.mhvcToRgb255(hue100, v, c)
