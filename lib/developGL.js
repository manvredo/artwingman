 ...
 * Uses Lab* distance: renders a red dot overlay at every pixel whose
 * Lab* is within tolerance of the reference pixel's Lab*.
 *
 * Returns a promise that resolves with [{x, y}, ...] positions.
 */
export function runMatchMask(state, refR, refG, refB, tolerance = 4.0) {
  ...
  for (let y2 = 0; y2 < h; y2++) {
    for (let x2 = 0; x2 < w; x2++) {
      if (pixels[(y2 * w + x2) * 4] > 128) {
        positions.push({ x: x2, y: y2 })
        nonzero++
      }
    }
  }
  console.warn('runMatchMask: refLab', refL.toFixed(1), refA.toFixed(1), refB2.toFixed(1), 'tolerance', tolerance, '→ match count', nonzero)
  return positions
}
