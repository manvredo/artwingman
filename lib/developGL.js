...
// Strategy: compare Lab distance instead of trying to replicate the full
// Munsell HVC conversion on GPU.  We pass the reference Lab* as uniforms,
// compute each pixel's Lab*, and mark it red if within Lab distance tolerance.
const FS_MATCH_MASK = `#version 300 es
precision highp float;
uniform sampler2D u_tex;
uniform vec3 u_refLab;   // reference Lab* (L*, a*, b*)
uniform float u_tLab;    // Lab distance tolerance
in vec2 v_uv; out vec4 o;
...
export function runMatchMask(state, refR, refG, refB, tolerance = 1.0, scale = 1) {
  ...
  // Compute reference Lab* from reference RGB (same analytical approach as shader)
  const rn = refR / 255, gn = refG / 255, bn = refB / 255
  ...
  console.warn('runMatchMask: refLab', refL.toFixed(1), refA.toFixed(1), refB2.toFixed(1), 'tol', tolerance, '→', nonzero, 'matches of', sw * sh)
  return positions
}
