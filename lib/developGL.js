...
precision highp float;
uniform sampler2D u_tex; uniform float u_amt;
in vec2 v_uv; out vec4 o;
void main() {
  vec3 c = texture(u_tex, v_uv).rgb;
  vec2 d = (v_uv - 0.5) * 2.0;
  float v = 1.0 - dot(d, d) * (u_amt * 0.004);
  o = vec4(clamp(c * v, 0.0, 1.0), 1.0);
}`
...
void main() {
  vec3 c = texture(u_tex, v_uv).rgb;
  float n = (rnd(v_uv) - 0.5) * (u_amt * 0.01);
  o = vec4(clamp(c + vec3(n, n, n), 0.0, 1.0), 1.0);
}`
...
const FS_CHROMA_GROUPS = `#version 300 es
precision highp float;
uniform sampler2D u_tex;
uniform float u_steps;
in vec2 v_uv; out vec4 o;
...
void main() {
  ...
  vec3 xyz = toXYZ(toLinear(rgb));
  float fx = f_lab(xyz.x / 0.95047);
  float fy = f_lab(xyz.y / 1.0);
  float fz = f_lab(xyz.z / 1.08883);
  float L = 116.0 * fy - 16.0;
  float a = 500.0 * (fx - fy);
  float b = 200.0 * (fy - fz);
  float chroma = sqrt(a*a + b*b);
  float group = min(u_steps - 1.0, floor(chroma * u_steps / 60.0));
  float t = group / max(1.0, u_steps - 1.0);
  float gray = 0.02 + t * 0.98;
  o = vec4(gray, gray, gray, 1.0);
}`
...
export function initGL(canvas) {
  ...
  const progs = {
    ...
    vignette: mkProg(gl, FS_VIGNETTE),
    noise:   mkProg(gl, FS_NOISE),
    lut:        mkProg(gl, FS_LUT),
    valueGroups: mkProg(gl, FS_VALUE_GROUPS),
    chromaGroups: mkProg(gl, FS_CHROMA_GROUPS),
...
export function runDevelop(state, params, dstCtx, sourceTex) {
  ...
  const {
    ...
    clarity=0, texture=0, sharpen=0, vignette=0, noise=0, lutIntensity=100,
  } = params
  ...
  if (vignette !== 0) {
    const dst = src ^ 1
    Q(progs.vignette, fbos[dst], { u_tex: texs[src] }, { u_amt: vignette })
    src = dst
  }
  ...
  if (noise !== 0) {
    const dst = src ^ 1
    Q(progs.noise, fbos[dst], { u_tex: texs[src] }, { u_amt: noise })
    src = dst
  }

  // Pass 7 — LUT (trilinear via GPU sampler3D)
  if (lutTex && lutSize > 1) {
    const dst = src ^ 1
...
export function runChromaGroups(state, steps, soften, dstCtx) {
  const { gl, vbo, progs, origTex, texA, texB, fboA, fboB, w, h } = state
  const Q = (prog, fbo, texs, f32s) => quad(gl, prog, vbo, w, h, fbo, texs, f32s)
  Q(progs.chromaGroups, fboA, { u_tex: origTex }, { u_steps: steps })
  if (soften > 0) {
    const scale = soften / 5  // soften=5 → 10px radius, soften=20 → 40px
    Q(progs.blurH, fboB, { u_tex: texA }, { u_tw: scale / w })
    Q(progs.blurV, fboA, { u_tex: texB }, { u_th: scale / h })
