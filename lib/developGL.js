...
export function initGL(canvas) {
  ...
  return { gl, canvas, vbo, progs, origTex, texA, texB, texC, fboA, fboB, fboC, w: 1, h: 1, lutTex: null, lutSize: 0 }
}

/** Upload a new image (HTMLImageElement). Resizes ping-pong textures if needed. */
export function uploadImage(state, imgEl) {
  const { gl, origTex, texA, texB, texC } = state
  ...
  for (const tex of [texA, texB, texC]) {
 ...
 */
export function runDevelop(state, params, dstCtx) {
  ...
  Q(progs.develop, fbos[0], { u_tex: origTex }, {
...
/** Quantize image into N luminance groups (grayscale), blit result to dstCtx. */
export function runValueGroups(state, steps, dstCtx) {
  const { gl, vbo, progs, origTex, texA, fboA, w, h } = state
  const Q = (prog, fbo, texs, f32s) => quad(gl, prog, vbo, w, h, fbo, texs, f32s)
