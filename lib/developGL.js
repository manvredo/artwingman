...
out vec2 v_uv;
void main() {
  // UNPACK_FLIP_Y_WEBGL=true flips texture on Y axis during upload.
  // v_uv.y needs to be flipped so shader sampling matches the flipped texture.
...
precision highp float;
uniform sampler2D u_tex;
uniform vec3 u_refLab;   // Erwartet L* [0..100], a* [-128..127], b* [-128..127]
uniform float u_tLab;    // Schwellwert (z.B. 4.0)
in vec2 v_uv; out vec4 o;

// D65 Weißpunkt-Konstanten
...
void main() {
  ...
  if (dist <= u_tLab) {
    o = vec4(1.0, 0.0, 0.0, 0.8);
  } else {
    o = vec4(0.0);
  }
}`

// JS helper for computing reference Lab (must match shader exactly)
// Returns [L, a, b] — XYZ is divided by white point INSIDE f(), same as shader
function rgbToLabJS(r, g, b) {
...
export function runMatchMask(state, refR, refG, refB, tolerance = 1.0, scale = 1) {
  const { gl, vbo, progs, origTex, matchMaskTex, matchMaskFBO, w, h } = state
  const sw = Math.ceil(w / scale)
  const sh = Math.ceil(h / scale)
  const isDownsample = scale > 1

  // Resize mask texture and FBO to target resolution
  if (isDownsample) {
  ...
  console.warn('runMatchMask: refLab', refLab.L.toFixed(1), refLab.A.toFixed(1), refLab.B.toFixed(1), 'tol', tolerance, '→', nonzero, 'matches of', sw * sh)
  return positions
}
